...
export default {
  ...
  methods: {
    fetchVocabulos() {
      axios
        .get('http://localhost:8080/vocabulo')
        .then((response) => {
    ...
    addVocabulo() {
      axios
        .post('http://localhost:8080/vocabulo', this.newVocabulo)
        .then(() => {
          this.fetchVocabulos();
          this.clearForm();
        })
    ...
    buscarPorTermoEVersao() {
      axios
        .get(
          `http://localhost:8080/vocabulo/termo/${this.termoConsulta}/versao/${this.versaoConsulta}`
        )
        .then((response) => {
          if (response.data) {
            this.vocabuloList = response.data.map((vocabulo) => {
